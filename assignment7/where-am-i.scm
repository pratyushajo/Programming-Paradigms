...
;; distance-product function
(define (distance-product point points-list)
  (apply * (map (lambda (value) (dist point value))
		(remove point points-list))))

;;rate-points function
(define (rate-points points-list)
  (map (lambda (value) (list (distance-point value points-list) value)) points-list))


;;sort-points function
(define (sort-points list)
  (quicksort list (lambda (one two) (< (car one) (car two)))))

;;clumped-points function
(define (clumped-points list)
  (apply append (map cdr (prefix-of-list (sort-points (rate-points list))
					 (quotient (length list) 2)))))

;;average-point function
(define (average-point list)
  (let (avg 
	(list (/ (apply + (map x list)) (length list))
	      (/ (apply + (map y list)) (length list))))
  (list (distance-product avg list) avg)))
...
;;where-am-i function
(define (where-am-i distance-list stars-list)
  (sort-points (map best-estimate (all-guesses distance-list stars-list))))
